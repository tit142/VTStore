...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='VTStore - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'truongtittit142@gmail.com';
            $config['smtp_pass']    = 'hekm hsok ypkq proc';
            ...
            $this->email->from('truongtittit142@gmail.com', 'VTStore');
            $this->email->to($email);
            $this->email->subject('Hệ thống VTStore - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng VTStore, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
                <br>Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!!');
            $this->email->send();
        ...
        $this->data['title']='VTStore - Đăng ký tài khoản';   
...
//laylaipass
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('VTStore', 'VTStore');
            $this->email->to($list['email']);
            $this->email->subject('VTStore - Lấy lại mật khẩu');
        ...
        $this->data['title']='VTStore - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='VTStore - Cập nhật mật khẩu mới';
