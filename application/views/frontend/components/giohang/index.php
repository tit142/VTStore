							...
							<div class="clearfix btn-submit" style="padding-left: 10px;margin-top: 20px;">
								<table class="table total-price" style="border: 1px solid #ececec;">
									<tbody>
										...
										if ($row['number'] <= $value) {
											echo '<td colspan="2">';
echo '<button disabled style="background-color:#c2d2cb" type="button" onclick="window.location.href=\'info-order\'" class="btn-next-checkout">Đặt hàng</button>';
echo '</td>';

										}else{
											echo '<td colspan="2">';
											echo '<button  type="button" onclick="window.location.href=\'info-order\'" class="btn-next-checkout">Đặt hàng</button>';
											echo '</td>';
										}
										?>
